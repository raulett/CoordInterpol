...
def getDATdataFromFile(file):
    ...
    # pattern = 20220609_130233.dat
    datetime_tuple = os.path.basename(fileDAT.name).split('.')[0].split('_')
    print("date_time_Expo = {}".format(datetime_tuple))
    # date_time = datetime.datetime.strptime(datetime_ExpoTime_tuple[0]+'T'+datetime_ExpoTime_tuple[1], '%Y%m%dT%H%M%S')+ datetime.timedelta(days=1, seconds=-1)
    date_time = datetime.datetime.strptime(datetime_tuple[0] + 'T' + datetime_tuple[1],'%Y%m%dT%H%M%S')
    print("parse datetime {}".format(date_time))
    expo_time = 2
    # expo_time = int(datetime_tuple[2])
